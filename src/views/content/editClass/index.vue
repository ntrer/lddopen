<template>
  ...
  <div class="app-container home">
    <el-page-header @back="goBack" class="my-header">
      ...
      <span slot="content" style="font-weight: bold;font-size: 16px;">修改课程</span>
    </el-page-header>
  ...
  export default {
    ...
    methods: {
      ...
      rowDrop() {
        ...
        const tbody=document.querySelectorAll('.el-tabs__content')[0].querySelectorAll('.rowform')[i]
     ...
     getTrainCourse(){
       getTrainCourse( this.providersCourseId).then(response => {
         console.log(response)
         this.form=response.data.providersTrainCourse
         let data = {
             url: response.data.providersTrainCourse.fullCourseImgUrl,
             relative_url: response.data.providersTrainCourse.courseImgUrl
           }
         this.imgList.push(data)

         if(this.trainTutorList.length!=0){
           for (var i = 0; i < this.trainTutorList.length; i++) {
               if(response.data.providersTrainCourse.tutorId==this.trainTutorList[i].tutorId){
                 this.selectImg = this.trainTutorList[i].fullImgUrl
                 this.teacherName = this.trainTutorList[i].tutorName
               }
           }
         }


         for (var i = 0; i < response.data.providersTrainChapterList.length; i++) {
           let newTabName = '第' + (i+1) + '章';
           this.editableTabs.push({
             title: newTabName,
             name: (i+1).toString(),
           });
           let data={
           chapterName:response.data.providersTrainChapterList[i].chapterName,
           providersChapterId:response.data.providersTrainChapterList[i].providersChapterId,
           providersTrainClassHourList:response.data.providersTrainChapterList[i].providersTrainClassHourList
           }
           this.TrainCourseLists.push(data)

         }
         this.editableTabsValue = response.data.providersTrainChapterList.length.toString();
         this.index=response.data.providersTrainChapterList.length
         this.tabIndex=response.data.providersTrainChapterList.length-1
         ...
         this.$nextTick(() => {
              this.index=this.TrainCourseLists.length-1
              this.rowDrop()
           })
         ...
         if (valid) {
           ...
           if(this.coverImgList.length!=0){
            ...
            this.TrainCourseLists[this.tabIndex].providersTrainClassHourList[this.selectIndex].fullCoverUrl=this.coverImgList[0].url
             ...
             // this.TrainCourseLists[this.tabIndex].providersTrainClassHourList[this.selectIndex].totalDuration='00:00:00'
     ...
     editClass(item){
      ...
      // 获取选择的导师
      chooseTutor(value) {
        console.log("啊实打实打算",value)
        if (value != null) {
          this.form.tutorId = value
          for (var i = 0; i < this.trainTutorList.length; i++) {
            if(value==this.trainTutorList[i].tutorId){
              this.selectImg = this.trainTutorList[i].fullImgUrl
              this.teacherName = this.trainTutorList[i].tutorName
            }
          }
      ...
      submitForm() {
        ...
        editTrainCourse(this.form).then(response => {
          console.log(response)
