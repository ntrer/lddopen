<template>
  ...
  <el-page-header @back="goBack" class="my-header">
    ...
    <span slot="content" style="font-weight: bold;font-size: 16px;">修改导师</span>
  </el-page-header>
  ...
  <div style="display: flex;flex-direction: row;">
   ...
   <bottom-card moduleName="内容" pagesName="编辑器材" style="margin-top: 20px;"></bottom-card>
...
<script>
  ...
  import {addTrainTutor,getTrainTutor} from "@/api/content/teacher";
  export default {
    name: "index",
    ...
    data() {
      return {
        ...
        providersTutorId:null
      };
    },

    created() {
      this.providersManufactorId = this.$route.query.dataId
      this.getDetail(this.providersManufactorId)
    },
    ...
    methods: {
       ...
       getDetail(id){
         getTrainTutor(id).then(response => {
           console.log(response)
           this.form=response.data.providersTrainTutor
           let data = {
               url: response.data.providersTrainTutor.fullTutorImgUrl,
               relative_url: response.data.providersTrainTutor.tutorImgUrl
             }
           this.imgList.push(data)
           this.teacherName=this.form.tutorName
           this.classIntroduction=this.form.introduction
           this.selectImg=response.data.providersTrainTutor.fullTutorImgUrl
         });
       },
