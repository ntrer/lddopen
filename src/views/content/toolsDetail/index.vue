<template>
  <div class="app-container home">
    <el-page-header @back="goBack" class="my-header">
      ...
      <span slot="content" style="font-weight: bold;font-size: 16px;">查看器材</span>
    </el-page-header>
          ...
          <div style="width: 100%; height: 140px; margin-top: 10px;display: flex; justify-content: space-between;">
            ...
            <div style="width: 50%;height: 120px; display: flex;">
              ...
              <div style="display: flex;flex-direction: column; margin-left: 15px;margin-right: 10px;">
                <span style="margin-top: 5px;font-weight: 450;font-size: 14px; ">{{productName}}</span>
                <div style=" height: 30px; margin-top: 20px; display: flex;justify-content: space-between;">
                  ...
                  <div style="margin-left: 140px; color: darkgray; font-size: 12px;">
                    厂商:
                    <span style="color: #000000; font-weight: 400;">{{manufactorName}}</span>
                  </div>
                </div>
                ...
                <div style="display: flex; width: 90px;height: 26px; border-radius: 5px;border: 1px solid #19AA29;">
                  ...
                  <el-dropdown style="justify-content: center;color: black;width: 35%;" >
                    ...
                    <el-dropdown-menu slot="dropdown">
                      <el-dropdown-item style="width: 100px;" v-if="data.sysAuditStatus==0"  @click.native="pushLine(data.providersProductId)">提交上线</el-dropdown-item>
                      <el-dropdown-item v-if="data.sysAuditStatus==1"  @click.native="downLine(data.providersProductId)">下线</el-dropdown-item>
                      <router-link :to="{name:'editTools',path: '/content/editTools',query:{ data: data }}" >
                        <el-dropdown-item command="3">修改</el-dropdown-item>
                      </router-link>
                      <el-dropdown-item v-if="data.sysAuditStatus!=-1" @click.native="reExemine(data.providersProductId)">撤回审核</el-dropdown-item>
                      <el-dropdown-item @click.native="delPro(data.providersProductId)">删除</el-dropdown-item>
                    </el-dropdown-menu>
                  </el-dropdown>
 ...
 <el-dialog :title="title" :visible.sync="openComment" width="600px" append-to-body class="my-dialog">
  ...
  <span style="font-size: 14px;">评论内容</span>
   ...
   <bottom-card moduleName="内容" pagesName="器材详情" style="margin-top: 20px;"></bottom-card>
...
<script>
  ...
  import {productList,productCount,ProductOppreate,dataOverview,dataOverviewTop} from "@/api/content/tools";
  ...
  export default {
    ...
    activated() {
     ...
     // 从前页进来
     if(this.data.providersProductId!=undefined){

       this.handleDetail(this.data.providersProductId)
     }
     // 从后页返回
     else{

        this.handleDetail2( localStorage.getItem('providersProductId'))
     }
    ...
    methods: {
      ...
      this.getOverView()
       ...
       // 显示详情
      handleDetail(providersProductId){

        // 获取上半部数据
        dataOverviewTop(providersProductId).then(response => {
            console.log(response)
            this.productName=response.data.productName
            this.classificationName=response.data.classificationName
            ...
            this.showViewNum=response.data.showViewNum||0
        });

        localStorage.setItem('providersProductId',providersProductId);
      },


      getOverView(){
        let params={
          providersProductId:this.data.providersProductId||localStorage.getItem('providersProductId'),
          startDate:this.selectTime[0],
          endDate:this.selectTime[1]
        }
        dataOverview(params).then(response => {
            console.log(response)
            this.getMap(response.data.viewnList)
      ...
      // 从下页返回
      handleDetail2(providersProductId){
